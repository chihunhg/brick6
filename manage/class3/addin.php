...
/**
 * class3 表單送出（結構同 class1/addin.php，主檔多 Class1_PKey、Class2_PKey）
 */
...
$csrfKey = (string)($detailConfig['csrf'] ?? 'class3_addin');
crud_csrf_verify_form($csrfKey);

global $filter_array, $file_array, $Class_Name;
...
$class1Pk = safe_int($filter_array['Class1'] ?? 0);
$class2Pk = safe_int($filter_array['Class2'] ?? 0);
if ($class1Pk <= 0) {
    $MSG .= '【' . (string)($Class_Name[1] ?? '大類') . "】請選擇\n";
}
if ($class2Pk <= 0) {
    $MSG .= '【' . (string)($Class_Name[2] ?? '中類') . "】請選擇\n";
}
...
$ForderName    = $ForderName ?? 'dbclass3_';
...
/* ── 主檔欄位 ───────────────────────────────────────── */
$Home = !empty($filter_array['Home']) ? 'Yes' : '';

$data_array = [
    $moduleCol    => SqlFilter($modulePKey, 'int'),
    'Class1_PKey' => SqlFilter($class1Pk, 'int'),
    'Class2_PKey' => SqlFilter($class2Pk, 'int'),
