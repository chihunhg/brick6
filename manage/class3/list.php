...
$listCsrfKey = 'class3_list';
$table_name  = (string)($detailConfig['master'] ?? 'dbclass3');
$PKName      = 'PKey';
$FKName      = (string)($detailConfig['fk'] ?? 'Class3_PKey');
...
[$PDO_Cond, $Cond_Array] = crud_module_where('t3');
...
$Keywords = crud_list_apply_keyword_search($PDO_Cond, $Cond_Array, $filter_array ?? [], 't3.strName', $kwPlaceholder);
if ($Keywords === '') {
    $Keywords = $kwPlaceholder;
}

$joinClass1 = (function_exists('chkTable') && chkTable('dbclass1'))
    ? ' LEFT JOIN dbclass1 c1 ON c1.PKey = t3.Class1_PKey'
    : '';
$joinClass2 = (function_exists('chkTable') && chkTable('dbclass2'))
    ? ' LEFT JOIN dbclass2 c2 ON c2.PKey = t3.Class2_PKey'
    : '';
$selectClass1 = $joinClass1 !== '' ? ', c1.strName AS class1_name' : ", '' AS class1_name";
$selectClass2 = $joinClass2 !== '' ? ', c2.strName AS class2_name' : ", '' AS class2_name";

$Total = crud_fetch_scalar(
    "SELECT COUNT(t3.{$PKName}) AS Total FROM {$table_name} t3{$joinClass1}{$joinClass2} {$PDO_Cond}",
    $Cond_Array,
    'Total'
);
...
$sql = 'SELECT t3.*' . $selectClass1 . $selectClass2
    . " FROM {$table_name} t3{$joinClass1}{$joinClass2} {$PDO_Cond}"
    . ' ORDER BY t3.Sort ASC, t3.dtUDate DESC LIMIT '
...
$listGridClass = manage_list_grid_class('class3');
$detailConfig = require __DIR__ . '/_config.php';
	...
	<div id="view-list">
		...
		<div class="card">
			...
			<div class="tableHeader <?php echo e($listGridClass); ?>">
				...
				<div><?php echo $Class_Name[1]?></div>
				<div><?php echo $Class_Name[2]?></div>
				<div>標題</div>
			...
			<div class="tableRow">
				...
				<div class="tableRow__item" data-id="<?php echo $rowPKey; ?>">
					<div class="tableRow__data <?php echo e($listGridClass); ?>">
						...
						<div><?php echo e((string)($row['class2_name'] ?? '')); ?></div>
						<div>
					...
					<div id="detail-<?php echo $rowPKey; ?>" class="tableRow__detail is-collapsed">
						<div class="tableRow__detail">
							...
							&nbsp;|&nbsp;
							<strong><?php echo e((string)($Class_Name[1] ?? '分類')); ?>：</strong><?php echo e((string)($row['class1_name'] ?? '')); ?>
							&nbsp;|&nbsp;
							<strong><?php echo e((string)($Class_Name[2] ?? '分類')); ?>：</strong><?php echo e((string)($row['class2_name'] ?? '')); ?>
						</div>
					</div>
					<?php } ?>
				</div>
