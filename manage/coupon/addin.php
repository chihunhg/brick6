...
$csrfKey = (string)($detailConfig['csrf'] ?? 'coupon_addin');
...
$MSG = coupon_validate_form($filter_array ?? [], $formPKey, $isEdit);
if ($MSG !== '') {
    crud_form_error_redirect($MSG, $returnUrl);
}

if ($isEdit && !coupon_verify_module_row($formPKey, $modulePKey)) {
    crud_form_error_redirect('查無要修改資料或無權限', $returnUrl);
}

$data_array = coupon_build_master_data($filter_array ?? [], $modulePKey, $Login_ID, $isEdit);
