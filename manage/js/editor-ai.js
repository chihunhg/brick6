...
(function ($) {
    ...
    function buildDefaultPrompt(editorId, hasSourceUrl, formatMode) {
        ...
        var formatHints = {
            prose: '撰寫圖文段落內文：以 h2、h3 分段，p 段落說明，strong 粗體、span style 設定顏色或字級強調重點。',
            table: '撰寫內文並以 HTML table 呈現規格、方案比較或重點整理；表頭用 thead/th、資料用 tbody/td，表格外可用 strong 與 span style 強調。',
            list: '撰寫條列式內文：以 h2 小標搭配 ul/ol 條列重點，strong 粗體與 span style 標示關鍵字。',
            auto: '撰寫官網內文：依內容混合 h2/h3、p、ul/ol、strong 粗體、span style 字級與顏色，有數據比較時使用 table。'
        };
    ...
    function handleGenerateClick(el, e) {
        ...
        window.manageGeminiStreamPost(resolveApiUrl(), payload, {
            onStart: function () {
                setLoadingOverlay(true, 'AI 正在串流產生內容…');
            },
            onDelta: function (delta, accumulated) {
                var partialHtml = window.manageGeminiTryPartialHtml(accumulated);
                if (partialHtml !== null && partialHtml !== '') {
                    setEditorHtml(editorId, partialHtml);
                }
                setLoadingOverlay(true, 'AI 產文中…（已接收 ' + accumulated.length + ' 字元）');
            },
            onDone: function (res) {
