    ...
    if (typeof AccessibilityManager !== 'undefined') {
        ...
        function toggleMenu(linkElement) {
            ...
            if (sub && hasSubElement) {
                ...
                // 檢查所有層級的 --isOpen 狀態
                var allSideNavDrops = document.querySelectorAll('.sideNavDrop');
                allSideNavDrops.forEach(function(drop, index) {
                    var isOpen = drop.classList.contains('--isOpen');
                    var parentMenuItem = drop.closest('.sideNav__item, .sideNavSub__item');
                    var parentHasSub = parentMenuItem ? parentMenuItem.querySelector('.--hasSub') : null;
                    var parentIsOpen = parentHasSub ? parentHasSub.classList.contains('--isOpen') : false;
                });
                
                // 檢查所有連結的 tabindex 狀態
                var allLinks = document.querySelectorAll('.sideNavDrop a, .sideNavDrop .txt');
                allLinks.forEach(function(link, index) {
                    var tabindex = link.getAttribute('tabindex');
                    var parentDrop = link.closest('.sideNavDrop');
                    var parentIsOpen = parentDrop ? parentDrop.classList.contains('--isOpen') : false;
                });
        ...
        function disableMenuFocus(dropElement) {
            if (dropElement) {
                ...
                currentElements.each(function(index, element) {
                    ...
                    var parentDrop = element.closest('.sideNavDrop');
                    var parentIsOpen = parentDrop ? parentDrop.classList.contains('--isOpen') : false;
                });
        ...
        function enableMenuFocus(dropElement) {
            if (dropElement) {
                ...
                // 記錄變更前的狀態
                directChildren.each(function(index, element) {
                    var tabindex = element.getAttribute('tabindex');
                    var parentDrop = element.closest('.sideNavDrop');
                    var parentIsOpen = parentDrop ? parentDrop.classList.contains('--isOpen') : false;
                });
