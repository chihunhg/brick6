...
$qiForm = is_array($GLOBALS['question_item_form'] ?? null) ? $GLOBALS['question_item_form'] : [];
$Question_PKey = (int)($qiForm['Question_PKey'] ?? 0);
$Question_D_PKey = (int)($qiForm['Question_D_PKey'] ?? 0);
$Question_Name = (string)($qiForm['Question_Name'] ?? '');
$Question_Class_Name = (string)($qiForm['Question_Class_Name'] ?? '');
$answerSlots = max(1, (int)($detailConfig['answer_slots'] ?? 10));
$strName = is_array($qiForm['strName'] ?? null) ? $qiForm['strName'] : question_item_empty_strname_slots();
$A_Name = is_array($qiForm['A_Name'] ?? null) ? $qiForm['A_Name'] : question_item_empty_answer_slots($answerSlots);
?><!DOCTYPE html>
...
$(function() {
	show_answer();
	$('#Qtype').on('change', show_answer);
});

function show_answer() {
	var qtype = parseInt($('#Qtype').val(), 10) || 0;
	var show = (qtype === 1 || qtype === 2);
	$('.js-answer-block').toggleClass('is-hidden', !show);
	$('#otherBlock').toggleClass('is-hidden', !show);
}
...
function fieldCheck0(theForm) {
	...
	if (qtype === 1 || qtype === 2) {
		var hasAnswer = false;
		for (var li = 1; li <= totalLang; li++) {
			for (var j = 1; j <= <?php echo (int)$answerSlots; ?>; j++) {
				if ($.trim($('#A_Name' + j + '_' + li).val() || '') !== '') {
					hasAnswer = true;
					break;
				}
			}
		...
		if (!hasAnswer) {
			errors.push('答案名稱至少填寫一項（請至少填寫一個語系）');
			array.push('A_Name1_1');
			view.push(1);
		}
	}
	if (errors.length) {
            ...
            <main class="mainContent">
                <div class="container">
                    ...
                    <section class="editView">
                        ...
                        <article class="editView__body">
                            <div class="editView__section">
                                ...
                                <div class="formGrid">
                                    ...
                                    <div class="col--10"><?php echo e($Question_Name); ?></div>
                                </div>
                                <div class="formGrid">
                                    <label class="col--2 inputLabel editView__formLabel">問卷類別</label>
                                    <div class="col--10"><?php echo e($Question_Class_Name); ?></div>
                                </div>
                                <div class="formGrid">
                                ...
                                <div class="formGrid">
                                    <label class="col--2 inputLabel editView__formLabel" for="Qtype">題型 <span class="inputLabel__required">*</span></label>
                                    <div class="col--10">
                                        <select name="Qtype" id="Qtype" class="formSelect">
                                            <option value="">請選擇</option>
                                            <?php for ($t = 1; $t <= 5; $t++) { ?>
                                            <option value="<?php echo $t; ?>"<?php echo (string)($Qtype ?? '') === (string)$t ? ' selected' : ''; ?>>
                                                <?php echo e(question_type($t)); ?>
                                            </option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="formGrid is-hidden" id="otherBlock">
                                    <label class="col--2 inputLabel editView__formLabel">其它顯示</label>
                                    <div class="col--10">
                                        <label class="flex flex--itCenter gap--2">
                                            <input name="Other" type="checkbox" id="Other" value="Yes"
                                                <?php if (($Other ?? '') === 'Yes') { echo 'checked'; } ?>>
                                            是，其它顯示
                                        </label>
                                    </div>
                                </div>
                                <div class="formGrid">
                                    <label class="col--2 inputLabel editView__formLabel">必填</label>
                                    <div class="col--10">
                                        <label class="flex flex--itCenter gap--2">
                                            <input name="Must" type="checkbox" id="Must" value="Yes"
                                                <?php if (($Must ?? '') === 'Yes') { echo 'checked'; } ?>>
                                            是，必填
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div id="tabCon_<?php echo $i; ?>" class="tabContent --color<?php echo $i; ?>">
                                    <div class="formGrid">
                                        <label class="col--2 inputLabel editView__formLabel" for="strName<?php echo $i; ?>">
                                            題目名稱 <span class="inputLabel__required">*</span>
                                        </label>
                                        <div class="col--10">
                                            <input name="strName<?php echo $i; ?>" type="text"
                                                id="strName<?php echo $i; ?>" class="formInput" maxlength="100"
                                                value="<?php echo e((string)($strName[$i] ?? '')); ?>">
                                        </div>
                                    </div>
                                    <div class="js-answer-block is-hidden">
                                        <div class="formGrid">
                                            <label class="col--2 inputLabel editView__formLabel">答案名稱 <span class="inputLabel__required">*</span></label>
                                            <div class="col--10">
                                                <div class="tableHeader tableGrid--question-answer">
                                                    <div class="textCenter">順序</div>
                                                    <div>名稱</div>
                                                </div>
                                                <?php for ($slot = 1; $slot <= $answerSlots; $slot++) { ?>
                                                <div class="tableRow__data tableGrid--question-answer">
                                                    <div class="textCenter"><?php echo $slot; ?></div>
                                                    <div>
                                                        <input name="A_Name<?php echo $slot; ?>_<?php echo $i; ?>"
                                                            id="A_Name<?php echo $slot; ?>_<?php echo $i; ?>"
                                                            type="text" class="formInput"
                                                            value="<?php echo e((string)($A_Name[$i][$slot] ?? '')); ?>">
                                                    </div>
                                                </div>
                                                <?php } ?>
                                            </div>
                        ...
                        echo hiddenNumeric('Question_D_PKey', (string)$Question_D_PKey) . PHP_EOL;
                        if (!$isAdd) {
