...
if ($ext === 'xls') {
    manage_alert_script('請改上傳 Excel 2007 以上的 .xlsx 檔', 'javascript:history.back()');
    exit;
}
if (!in_array($detected, $allowedMimes, true) && !in_array($ext, ['xlsx', 'csv'], true)) {
    manage_alert_script('檔案格式有誤，請上傳 Excel（.xlsx）', 'javascript:history.back()');
    exit;
}
...
try {
    $emails = xlsx_read_column_a($savePath, 2);
    $allRows = ($ext === 'csv') ? [] : xlsx_read_rows($savePath);
    $lines = ($ext === 'csv') ? count($emails) : max(0, count($allRows) - 1);
} catch (Throwable $e) {
    @unlink($savePath);
    manage_alert_script('無法讀取 Excel 檔案，請使用 .xlsx', 'javascript:history.back()');
    exit;
}
@unlink($savePath);
