...
// _list.php — 列表列渲染（需由 list.php 提供 $listRows、$Layer、$PKName、$i）
...
$layerSuffix = max(1, min(5, $Layer));
$gridClass = manage_list_expand_enabled()
    ? ('tableGrid--album-l' . $layerSuffix)
    : ('tableGrid--album-l' . $layerSuffix . '-compact');
if ($showHomeColumn) {
    ...
    <div class="tableRow__item" data-id="<?php echo $rowPKey; ?>">
        <div class="tableRow__data <?php echo e($gridClass); ?>">
            ...
            <div class="textCenter">
                <button type="button" class="btnStyle btnStyle--sm btnStyle--outline"
                    data-manage-action="manage-update"
                    data-page="../album_d/list.php"
                    data-pkey="<?php echo $rowPKey; ?>">
                    <i class="bi bi-pencil-square"></i> 設定
                </button>
            </div>
        </div>
        <?php if (manage_list_expand_enabled()) { ?>
