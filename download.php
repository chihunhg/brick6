...
/**
 * 前台檔案下載列表（download.htm / download{N}.htm）
 *
 * 資料流程：註冊模組設定 → frontend_fetch_list → frontend_filedown_items_from_rows。
 * 後台：manage/filedown/_config.php。
 */
 ...
 *
 * 使用方式：
 *   1. frontend_module_pkey_for_page('download.htm') — 依 PageLink 從後台選單反查 Module_PKey
 ...
 */
$Module_PKey = frontend_module_pkey_for_page('download.htm');
frontend_module_set_config(array_merge(
    require __DIR__ . '/manage/filedown/_config.php',
    [
        'view'                    => 'view_filedown',
        'class_link'              => 'download',
        'detail_link'             => 'download',
...
$downloadItems = frontend_filedown_items_from_rows($listRows);

$listTitleAll = (string)($Module_Name !== '' ? $Module_Name : '檔案下載');
            ...
            <ul class="downloadList">
                ...
                <li class="downloadList__item">
                    ...
                    <a href="<?php echo href_attr((string)$item['href']); ?>" class="downloadList__icon"
                        ...
                        title="<?php echo e_attr('下載 ' . $item['title']); ?>"
                        aria-label="<?php echo e_attr('下載 ' . $item['title']); ?>"
                        <?php if (!empty($item['is_file'])) { ?> download<?php } ?>>
