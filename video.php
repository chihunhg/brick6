...
/**
 * 前台影音列表（video.htm / video{N}.htm）
 *
 * 資料流程：註冊模組設定 → frontend_list_where → 分頁列表。
 * 後台：manage/video/_config.php。
 */
 ...
 *
 * 使用方式：
 *   1. frontend_module_pkey_for_page('video.htm') — 依 PageLink 從後台選單反查 Module_PKey
 ...
 */
$Module_PKey = frontend_module_pkey_for_page('video.htm');
frontend_module_set_config(array_merge(
    require __DIR__ . '/manage/video/_config.php',
    [
        'view'                    => 'view_video',
        'class_link'              => 'video',
        'detail_link'             => 'video-detail',
...
$listTitleAll = (string)($Module_Name !== '' ? $Module_Name : '所有資訊');
        ...
        <div class="container">
            ...
            <div class="imgCardList imgCardList--pg --news">
                ...
                foreach ($listRows as $row) {
                    ...
                    $movielink = (string)crud_row_val($row, 'Movielink');
                    $displayImgUrl = frontend_video_card_image_url($pkey, $movielink);
                    $watchHref = frontend_video_watch_href($movielink);
                ?>
                <?php if ($watchHref !== null) { ?>
                <a href="<?php echo e_attr($watchHref); ?>" class="imgCardList__item" target="_blank"
                    rel="noopener noreferrer" title="<?php echo e_attr($title); ?>">
                <?php } else { ?>
