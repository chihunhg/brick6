	...
	<div class="inputGroup__box__input">
		<div class="formMode">
			...
			<div class="form-check">
				<input id="<?php echo 'Q_'.$q.'_'.$a.'_'.$i?>" type="radio" class="form-check-input js-q-answer-input" name="<?php echo 'Q_'.$q.'_'.$a?>" value="<?php echo $as1->field('strName')?>">
				<label for="<?php echo 'Q_'.$q.'_'.$a.'_'.$i?>"><?php echo $as1->field('strName')?></label>
				<?php
				if($rs1->field('Other')=='Yes' && $i==$answer){
				?>
				<input id="<?php echo 'Q_'.$q.'_'.$a.'_'.($i+1)?>" type="radio" class="form-check-input js-q-answer-input" name="<?php echo 'Q_'.$q.'_'.$a?>" value="其它">
	...
	function updateSort() {
		...
		for (var n = 1; n <= total; n++) {
			if ($('#' + prefix + '_' + n).prop('checked')) {
				str.push(n);
				$('#' + prefix + '_Memo').prop('readOnly', false);
			} else {
				$('#' + prefix + '_Memo').prop('readOnly', true);
				$('#' + prefix + '_Memo').val('');
			}
		}
